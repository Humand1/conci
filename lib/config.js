...
export const REDASH_CONFIG = {
  base_url: process.env.REDASH_BASE_URL || 'https://redash.humand.co/api/queries',
...
// Función para obtener credenciales de autenticación
export const getAuthCredentials = () => {
  const user = process.env.HUMAND_API_USER
  const password = process.env.HUMAND_API_PASSWORD
  
  if (!user || !password) {
    throw new Error('Credenciales de Humand no configuradas. Configure HUMAND_API_USER y HUMAND_API_PASSWORD')
  }
...
export const validateConfig = () => {
  ...
  // Validar credenciales de Humand
  try {
    getAuthCredentials()
  } catch (error) {
    errors.push('Credenciales de Humand no configuradas')
  }
