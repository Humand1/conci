  ...
  async uploadDocument(userId, fileBuffer, filename, options = {}) {
    ...
    try {
      ...
      console.log(`📍 Coordenadas: ${signatureCoordinates ? 'SÍ' : 'NO'}`)
        ...
        // Preparar JSON payload según documentación oficial de Humand
        const requestBody = {
          file: fileBuffer.toString('base64'), // Archivo como base64
          folderId: folderId, // Como number
          name: filename,
          sendNotification: sendNotification, // Como boolean
          signatureStatus: signatureStatus,
          signatureCoordinates: signatureCoordinates || [], // Como array
        ...
        // DEBUG: Mostrar todos los campos del JSON payload
        console.log('\n📋 CAMPOS DEL JSON PAYLOAD:')
        console.log(`  - file: [Base64 PDF] ${filename} (${fileBuffer.length} bytes -> ${requestBody.file.length} chars base64)`)
        console.log(`  - folderId: ${folderId} (number)`)
        console.log(`  - name: "${filename}"`)
        console.log(`  - sendNotification: ${sendNotification} (boolean)`)
        console.log(`  - signatureStatus: "${signatureStatus}"`)
        console.log(`  - allowDisagreement: ${false} (boolean)`)
        console.log(`  - API key en headers: "${apiKey.substring(0, 20)}..."`)
        if (signatureCoordinates) {
          console.log(`  - signatureCoordinates: ${JSON.stringify(signatureCoordinates)} (array)`)
        } else {
          console.log(`  - signatureCoordinates: [] (array vacío)`)
        }
