...
class HumandAPIClient {
  ...
  async uploadDocument(userId, fileBuffer, filename, options = {}) {
    ...
    try {
      ...
      console.log('\n' + '='.repeat(80))
      console.log('🚀 INICIANDO SUBIDA A API DE HUMAND')
      console.log('='.repeat(80))
      console.log(`📁 Archivo: ${filename}`)
      console.log(`👤 Usuario ID: ${userId}`)
      console.log(`🌐 URL completa: ${fullUrl}`)
      console.log(`📂 Folder ID: ${folderId}`)
      console.log(`✉️ Notificación: ${sendNotification}`)
      console.log(`✍️ Estado firma: ${signatureStatus}`)
      console.log(`📍 Coordenadas: ${signatureCoordinates ? 'SÍ' : 'NO'}`)
      
        // Preparar form data usando camelCase como en el script Python que funciona
        const formData = new FormData()
        formData.append('file', new Blob([fileBuffer], { type: 'application/pdf' }), filename)
        formData.append('folderId', String(folderId))  // Corregido: usar camelCase como Python
        formData.append('name', filename)
        formData.append('sendNotification', sendNotification ? 'true' : 'false')  // Corregido: usar camelCase como Python
        formData.append('signatureStatus', signatureStatus)
        formData.append('allowDisagreement', 'false')
        
        // API key solo en headers, NO en FormData (como en Python)
        const apiKey = getHumandAPIKey()
      
        // signatureCoordinates debe ser un STRING JSON (usando camelCase como Python)
        if (signatureCoordinates) {
          formData.append('signatureCoordinates', JSON.stringify(signatureCoordinates))
          console.log(`📍 Coordenadas JSON: ${JSON.stringify(signatureCoordinates)}`)
        }
      
        // DEBUG: Mostrar todos los campos del FormData (corregido para camelCase)
        console.log('\n📋 CAMPOS DEL FORMDATA:')
        console.log(`  - file: [Blob PDF] ${filename}`)
        console.log(`  - folderId: "${folderId}"`)
        console.log(`  - name: "${filename}"`)
        console.log(`  - sendNotification: "${sendNotification ? 'true' : 'false'}"`)
        console.log(`  - signatureStatus: "${signatureStatus}"`)
        console.log(`  - allowDisagreement: "false"`)
        console.log(`  - API key en headers: "${apiKey.substring(0, 20)}..." (NO en FormData)`)
        if (signatureCoordinates) {
          console.log(`  - signatureCoordinates: ${JSON.stringify(signatureCoordinates)}`)
        }
      
      console.log('\n⏳ Enviando request a la API de Humand...')
      
        const response = await this.client.post(endpoint, formData, {
          headers: {
            // Remover Content-Type explícito - dejar que axios lo configure automáticamente con boundary
            // Esto es crítico para que funcione correctamente el multipart/form-data
          },
      ...
      console.log('\n❌ ERROR EN SUBIDA A API DE HUMAND')
      console.log('='.repeat(50))
      console.log(`⏱️ Duración: ${duration}ms`)
      console.log(`📁 Archivo: ${filename}`)
      console.log(`👤 Usuario ID: ${userId}`)
      ...
      if (error.response) {
        ...
        console.log(`📋 Headers de error:`)
        Object.entries(error.response.headers || {}).forEach(([key, value]) => {
          console.log(`    ${key}: ${value}`)
        })
        console.log(`📄 Response body de error:`)
