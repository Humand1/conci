  ...
  async uploadDocument(userId, fileBuffer, filename, options = {}) {
    ...
    try {
      ...
      console.log(`📍 Coordenadas: ${signatureCoordinates ? 'SÍ' : 'NO'}`)
        ...
        // Preparar FormData (multipart/form-data) según documentación de Humand
        const formData = new FormData()
        formData.append('file', new Blob([fileBuffer], { type: 'application/pdf' }), filename)
        formData.append('folderId', String(folderId))  // Como string según documentación
        formData.append('name', filename)
        formData.append('sendNotification', sendNotification ? 'true' : 'false')  // Como string
        formData.append('signatureStatus', signatureStatus)
        formData.append('allowDisagreement', 'false')  // Como string
        
        // API key solo en headers
        const apiKey = getHumandAPIKey()
      
        // signatureCoordinates debe ser un STRING JSON según documentación
        ...
        console.log('\n📋 CAMPOS DEL FORMDATA:')
        console.log(`  - file: [Blob PDF] ${filename} (${fileBuffer.length} bytes)`)
        console.log(`  - folderId: "${folderId}" (string)`)
        console.log(`  - name: "${filename}"`)
        console.log(`  - sendNotification: "${sendNotification ? 'true' : 'false'}" (string)`)
        console.log(`  - signatureStatus: "${signatureStatus}"`)
        console.log(`  - allowDisagreement: "false" (string)`)
        console.log(`  - API key en headers: "${apiKey.substring(0, 20)}..."`)
        if (signatureCoordinates) {
          console.log(`  - signatureCoordinates: ${JSON.stringify(signatureCoordinates)} (JSON string)`)
        }
      ...
      console.log('\n⏳ Enviando request a la API de Humand...')
        ...
        const response = await this.client.post(endpoint, formData, {
          headers: {
            // No especificar Content-Type para que axios configure automáticamente multipart/form-data
          },
