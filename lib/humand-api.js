...
class HumandAPIClient {
  ...
  async getSegmentations() {
    ...
    try {
      ...
      const segmentations = await this.processSegmentations(response.data)
  ...
  async processSegmentations(rawData) {
    const segmentations = []
    ...
    try {
      ...
      // Procesar cada grupo
      for (const [i, groupData] of rawData.entries()) {
        ...
        for (const [j, itemData] of itemsData.entries()) {
          ...
          // Obtener conteo real de usuarios para este item
          let realUserCount = 0
          try {
            console.log(`DEBUG: Obteniendo conteo de usuarios para item ${itemId}...`)
            const users = await this.getSegmentationUsers([String(itemId)], 0) // limit 0 para obtener solo el conteo
            realUserCount = users.length
            console.log(`DEBUG: Item ${itemId} tiene ${realUserCount} usuarios`)
          } catch (error) {
            console.warn(`DEBUG: Error obteniendo usuarios para item ${itemId}:`, error.message)
            realUserCount = itemData.usersCount || 0 // Fallback al valor original
          }
