...
class HumandAPIClient {
  ...
  async uploadDocument(userId, fileBuffer, filename, options = {}) {
    ...
    try {
      ...
      // Preparar form data exactamente como DYLO para compatibilidad completa
      const formData = new FormData()
      formData.append('file', new Blob([fileBuffer], { type: 'application/pdf' }), filename)
      formData.append('folder_id', String(folderId))  // Cambio: folderId -> folder_id (snake_case)
      formData.append('name', filename)
      formData.append('send_notification', sendNotification ? 'true' : 'false')  // Cambio: sendNotification -> send_notification (snake_case)
      formData.append('signatureStatus', signatureStatus)
      formData.append('allowDisagreement', 'false')
      
      // Agregar API key como DYLO
      const apiKey = getHumandAPIKey()
      ...
      console.log(`  - send_notification: "${sendNotification ? 'true' : 'false'}"`)
      console.log(`  - signatureStatus: "${signatureStatus}"`)
      console.log(`  - allowDisagreement: "false"`)
      console.log(`  - api_key: "${apiKey.substring(0, 20)}..."`)
      if (signatureCoordinates) {
        console.log(`  - signature_coordinates: ${JSON.stringify(signatureCoordinates)}`)
      }
